...
//     // Append the message to the console popup
//     consolePopup.appendChild(messageElement);

//     // Ensure the console scrolls to the bottom for the latest message
//     consolePopup.scrollTop = consolePopup.scrollHeight;
// }

function logMessageToPopup(message, duration = 3000) {
    const consolePopup = document.getElementById("console-popup");
    ...
    // Append the new message to the console popup
    consolePopup.appendChild(messageElement);

    // Auto-clear the message after the specified duration (default: 3 seconds)
    // setTimeout(() => {
    //     consolePopup.innerHTML = "";
    // }, duration);
}
...
function triggerEventBasedOnTerrain(tile) {
    switch (tile.type) {
        case 'grass':
            logMessageToPopup("You encounter a wandering merchant!");
            break;
        case 'mountain':
            logMessageToPopup("You discover a hidden cave!");
            break;
        case 'water':
            logMessageToPopup("You find a sunken treasure!");
            break;
        default:
